    ...
    def download_all(
        ...
        for link in self.video_urls:
            try:
                ...
                logger.info('downloading %s/%s', self.video_urls.index(link) + 1, len(self.video_urls))
                if prefix_number:
                    prefix = next(prefix_gen)
                    logger.debug('file prefix is: %s', prefix)
                    dl_stream.download(download_path, filename_prefix=prefix, skip_existing = skip_existing)
                else:
                    dl_stream.download(download_path, skip_existing = skip_existing)
